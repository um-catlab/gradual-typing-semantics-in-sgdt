...
(non-guarded) arities $\tau_\oper : \mathbb N$ for each $\oper \in \Sigma$. Then
we can form the type of finite trees with node types given by the signature:
%
\[ \textrm{FinTree}_\Sigma X = \mu T. X + ((\oper : \Sigma) \times T^{\tau_\oper}) \]
%
...
Then the globalization of a free algebra for $\calAs$ is a certain kind of
coinductive ``interaction tree'':
...
only the $\theta$ operation is guarded, there can be only finitely depth of
operations from the signature unless they are interrupted by a $\theta$. We
define $M \leadsto N$ to mean that $M = \textrm{leaf}(\delta(N))$, representing
that $M$ ``steps'' to $N$ in one internal ($\tau$) step.

Then the globalization of the strict error ordering $\sqsubseteq$ and weak
bisimilarity $\bisim$ for ITrees imply the following coinductively generated
...
\begin{theorem}[Graduality Schema for a Free Theory]\label{thm:gs-free-theory}
  Let $\calA$ be a free ordinary algebraic theory. Then there is a graduality
  schema for $\calA$.
...
We consider three examples utilizing our framework, in increasing order of complexity.
%
\begin{example}[Empty Theory]
  First, consider the empty theory $\calA = (\emptyset, \emptyset)$. Since
  $\calA$ is in particular a free theory, we obtain by Theorem
  \ref{thm:gs-free-theory} a graduality schema for $\calA$. Moreover, we can
  characterize the data of this schema concretely. We have $\calAs = \error +
  \steptheory$, the coproduct of the theory of errors with the theory of steps.
  The free-model monad is given by $T(X) = X + \{ \mho \} + \later (T(X))$, with
  the ordering and bisimilarity relations being lock-step error ordering and
  weak bisimilarity respectively as defined in the model of Giovannini et al
  \cite{gtt-sgdt}. Then we obtain the expected graduality theorem for the syntax:
\end{example}

\begin{example}[Global State]
  Now suppose $\calA := \state{\bool}$.
\end{example}

\begin{example}[Several Effects]

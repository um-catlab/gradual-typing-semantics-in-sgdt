...
%
The monad $\Ts$ supports each of the operations of the input theory $\calA$, in
addition to a nullary error operation $\mho$ and a guarded step operation
$\theta^s : (\later \Ts(X)) \to \Ts(X)$. This means that for each operation
$\oper$, of arity $\tau_\oper$, there is a function $\sem{\oper} : (\tau_\oper
\to \Ts X) \to \Ts X$.
%
...
imply that the relation $\ltbisim_E$ holds on collapsed elements.
%
...
the graduality result also translates without issue.

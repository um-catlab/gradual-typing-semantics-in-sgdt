...
The main contribution of this paper is a framework for giving an
adequate operational semantics to an effectful gradually-typed cast
calculus\max{and proving graduality surely!}. The input to the
framework consists of a specification of the algebraic effects present
in the cast calculus, given in the form of an \emph{algebraic theory},
which contains the operations along with equations that they
satisfy. More details on algebraic theories are given in Section
\ref{sec:modeling-effects}. The cast calculus contains a term
corresponding to each of the operations present in the provided
algebraic theory $\calA$, allowing the programmer to perform the
...
$M$ does not result in an infinite sequence of bureaucratic steps.\max{I think we can just say something specific here like ``bureaucratic steps are things like beta reductions that are invisible in the denotational semantics''}
...
We introduce two specific guarded algebraic theories that will be used throughout
the paper.
...
\begin{comment}
  ...
  The pure reductions are labeled by a natural number $i \in \{0, 1\}$, which
...
casts\max{limitation is maybe the wrong word. Just say it is a mismatch with the step counting in the operational semantics}. While this limitation was not problematic for their result, this change
...
It turns out\max{Don't start a paragrpah with ``It turns out''. Say something more active like ``their proof cleanly generalizes''} that the construction of perturbations and quasi-representable
...
For concreteness, recall\max{is ``recall'' the correct word here? Did we say this earlier or do we expect them to have read the previous paper} that in the prior model, term precision $M \ltdyn N :
A$ is modeled by $\sem{M} \ltbisim \sem{N}$ where
%
...
trace ending in a value or opponents move, then $N$ must exhibit the
same trace.

...
ordinary statement about the coinductive semantics $\run{-}$.  This
...
%% $\calA$.
We package all of the input data for our construction above into a
``graduality schema'' consisting of an algebraic theory, lifting of
the free algebras to act on relations and final global ordering
relations. In order to show the applicability of our framework, we
give two methods for constructing graduality schemata.
...
schemata for common monad transformers. So starting from an
...
\begin{enumerate}
    ...
    \item In Section \ref{sec:syntax}, we specify the syntax for the cast calculus and definitions of type and term precision used to define the graduality property.
...
\begin{example}[Reader]\label{ex:reader} The theory of immutable state
  $\calA_{\reader_R}$ models computations that can read from a single, global
  ...
  %
  \[ F_{\calA_{\reader_R}}(X) \cong R \to X. \]
   
\end{example}

\begin{example}[State]\label{ex:state} %The theory of stateful computations
    The theory of global state $\calA_{\state_S}$ for state set $S$ models
    ...
    $\Sigma_{\state}$ with the following operations:
    %
    \begin{itemize}
    ...
    %
    \[ F_{\calA_{\state_S}}(X) \cong S \Rightarrow (S \times X). \]
...
\begin{example}
  The theory $\calA_{\state} \otimes \calA_{\step}$ models computations that can
...
\th{\exn{E}}$. Each theory transformer $M$ induces a corresponding \textbf{monad
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    Exceptions   & $- + \th{\exn{E}}$         & $T'(A) = T(A + E)$ \\
    Reader       & $- \otimes \th{\reader_R}$ & $T'(A) = R \to T(A)$ \\
    Writer       & $- \otimes \th{\writer_W}$ & $T'(A) = T(W \times A)$ \\
    Global State & $- \otimes \th{\state_S}$  & $T'(A) = S \to T(S \times A)$ \\
    \hline
  \end{tabular}

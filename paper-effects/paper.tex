...
\begin{enumerate}
  ...
  \ref{sec:small-step-semantics}, and then extract a big-step $\run$ function as
  discussed in Section \ref{sec:big-step-semantics}. This uses the guarded
  extensions $\calAs$ and $\calAsb$ and their associated monads $\Ts$ and $\Tsb$.
  ...
  predomains and $\calAs$-domains. The perturbations and quasi-representability
  are the same as in the prior guarded model. The definition of the dynamic
  type is also as before, $D \cong \mathbb{N} + D \times D + \later U (D \to F D)$.
  ...
  \item We then map the bureaucratic delays in $\run M$ and $\run N$ to true delays.
  Then $\equiv_b$ turns into $\bisim$:
  \[ d(\run M) \bisim \sem{M} \bisim f \ltdyn g \bisim \sem{N} \bisim d(\run N). \]   
  \item We perform globalization and extensional collapse.
  \[ \gl(d(\run M)) \bisim^{gl} \gl(\sem{M}) \bisim^{gl} \gl(f) \ltdyn^{gl} \gl(g) \bisim^{gl} \gl(\sem{N}) \bisim^{gl} \gl(d(\run N)). \]

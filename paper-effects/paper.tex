...
Let $\calA$ be an ordinary algebraic theory. An $\calA$-graduality schema
consists of the following data:
\begin{itemize}
\item A guarded extension $\calAs \in \calA[\mho, \theta]$ and $\calAsb \in
\calA[\mho, \theta^s, \theta^b]$.
\item A presentation of the free models for $\calAs$, given by an adjunction 
$F \dashv U$ between sets and models of $\calAs$.
Likewise for $\calAsb$. Let $\Ts$ and $\Tsb$ be the respective monads.
\item A lifting of the adjunction for $\calAs$ to a strong adjunction between
the double categories of predomains and $\calAs$-domains. Let $T := UF$ be the
associated monad on predomains and let $T^{gl}(A) := \forall k.T^k(A)$.
\item For globalization: $T$ commutes with clock quantification: the canonical
map $T(\forall k.X[k]) \to \forall k. T(X[k])$ is an isomorphism.
\item For the extensional collapse: A family of types $E(A)$ along with
functions $\col : T^{gl}(A) \to E(A)$, along with...
\item ...A partial order $\ltbisim$ on each $E(A)$, such that...
\item ...$x \ltdyn^{gl} y$ and $x \bisim^{gl} y$ imply $\col{x} \ltbisim \col{y}$.
\end{itemize}

Given this data, we prove graduality and adequacy as follows:
\begin{enumerate}

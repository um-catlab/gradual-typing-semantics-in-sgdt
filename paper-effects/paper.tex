...
methodology of Kavvos \cite{kavvos-2025}. While this approach is applicable to
any algebraic theory representing the programming language's effects, what is
...
manipulations required in step-indexing.
...
\begin{itemize}
  ...
  \item There is a means of solving guarded-recursive type equations $T \cong
  F(\later T)$. Typically, this takes the form of a \emph{guarded fixpoint}
  operator
  %
...
effects. Our syntax is call-by-value and is presented in Figure
\ref{fig:gtlc-syntax}.
...
Lastly, we have terms $\synop{op}{V}{x}{M}$ corresponding to each operation in the
algebraic theory $T$. More concretely, if $\eff{op}{\sigma}{\tau}$ is an
operation in $T$ and $M$ is of type $\sigma$, then $\synop{op}{V}{x}{M}$ is of type
$\tau$.
...
\section{Modeling Effects}\label{sec:modeling-effects}
...
First, we recall the notions of a signature and an algebraic theory.
% TODO cite paper on algebraic effects

A \emph{signature} $\Sigma$ consists of a collection of operations $op : \sigma
\leadsto \tau$. Here, $\sigma$ is the \emph{request type} of operation $op$,
i.e., the type of the value that the program sends to the environment.
Similarly, $\tau$ is the \emph{response type} of the operation, i.e., the type
of the value that the environment sends back to the program once the operation
is complete. More formally, we write $\Sigma = (\Op, \req, \resp)$ where $\Op$
is a type, and $\req$ and $\resp$ are families of types indexed by $\Op$.

Given a signature $\Sigma$ as described above, we can form the \emph{polynomial
endofunctor} corresponding to $\Sigma$, which is defined by 
%
\[ \Sigma(X) := \Sigma_{\oper : \sigma \leadsto \tau \in \Op} (\sigma \times (\tau \to X)). \]
%

A $\Sigma$-structure is simply an algebra of the endofunctor $\Sigma$ defined
above. More concretely, a $\Sigma$-structure consists of a set $B$ equipped
with interpretations for each of the operations in $\Sigma$. That is, for each
$\oper : \sigma \leadsto \tau \in \Op$, there is a function $\sem{\oper} :
(\sigma \times (\tau \to B)) \to B$.
%
...
% We will often write B to denote either a $\Sigma$-structure or its underlying
% set.

Given a signature $\Sigma$ and a set $X$ (to be thought of as variables), we
define by induction the \emph{free} $\Sigma$-structure on $X$, which we denote
$\text{Syn}(X)$.
% Elements of $\text{Syn}(X)$ are $\Sigma$-terms over the set $X$: either
% variables in $X$, or trees consisting of a node labeled by an element of
% $\sigma$, and $\tau$-many children. 
This has the universal property that given any $\Sigma$-structure $B$ and any
map of variables $f : X \to B$, there is a unique map $\text{Syn}(X) \to B$
extending $f$. In other words, if we specify how to interpret the variables $X$
in $B$, this extends uniquely to an interpretation of any term.

An \emph{algebraic theory} $\calA$ consists of a signature $\Sigma_A$ and a
collection of equations $E_A$. A \emph{model} of an algebraic theory $\calA$ is
a set $B$ with an interpretation of the operations in $\Sigma_A$, such that all
equations in $E_A$ hold. Formally speaking, this means that when we interpret
the LHS and RHS of each equation as elements of $B$, those elements are equal.

Given two theories $\calA_1$ and $\calA_2$, we define their \emph{coproduct}
$\calA_1 + \calA_2$ by combining the operations and the equations in $\calA_1$
and $\calA_2$. We do not impose any additional equations.
...
For an algebraic theory $\calA$, the category of $\calA$-models has a forgetful
functor to the category of sets, which simply takes the underlying set of the
model. This functor has a left adjoint, which given a set $X$ constructs the
\emph{free model} of the algebraic theory $\calA$ with variables in $X$.
Concretely, the free model of $\calA = (\Sigma_A, E_A)$ on a set $X$ is given by
quotienting the set of $\Sigma_A$-terms by the equations in $E_A$.
%
This adjunction induces a monad on the category of sets, which is known as the
\emph{free model monad}.
% TODO cite
...
The implication of this is that the categories involved in the adjunction
described above are not merely the category of sets and the category of
$A$-models. We require additional relational structure: a relation to model the
graduality ordering as well as a notion of weak bisimilarity that relates terms
that differ only in the number of steps they take. These relations are crucial
ingredient in the denotational graduality proof. We defer detailed discussion of
these aspects to the section on denotational semantics; for now, it is
sufficient to keep in mind that the categories involved, and the free-model
monad $T_A$ for a given algebraic theory $A$, enjoy this additional structure.
...
terms of type $A$ in the object language, and likewise $\val{A}$ denotes the
type of closed values of type $A$ in the object language.

To model small-step reduction, we define a function
%
...
$\bstep$ represents an administrative step.
%
...
Having defined $\step$, we then extract a big-step operational semantics from
the small-step semantics:
%
\[ \run : \term{A} \to T_{db} (\val{A}). \]
%
Observe that the output of running a term is a monadic value. The monad accounts
for the all possible effects that may occur as the term is being run, including
stepping.
...
the $\step$ function whose signature we have discussed above. In particular:

\begin{itemize}
  \item A reduction rule $M \stepsin{k} \hat{N}$, where $\hat{N}$ is of type
  $T(\term{A})$, should be translated as $\step(M) = \inr (\hat{N}, t)$ where $t
  = \tstep$ if $k = 1$ and $t = \bstep$ if $k = 0$. That is, the number $k$
  (either $0$ or $1$) determines whether the step is considered a true step or
  an administrative step.

  \item A \emph{pure reduction rule} is a special case of the above where
  $\hat{N} = \eta N$ for some term $N$, where $\eta$ is the unit of the monad.
  In this case, we write $M \stepsin{k} N$, where $N$ is a term. This should be
  translated as $\step(M) = \inr (\eta (N))$.

  \item All other rules are \emph{impure reduction rules}.
  
  \item Any term $M$ that does not match one of these rules is equal to a value
  $V$, and hence $\step(M)$ returns the value $V$ and the proof that $M$ is
  equal to $V$.
\end{itemize}
...
steps to the original value. Otherwise, the term steps to the error element of
the monad (i.e., this is an impure reduction). Note that when downcasting a
dynamically-typed function, we record one recursive unfolding. In the
denotational semantics, this corresponds to inserting a $\theta$.
%
...
administrative steps. It is necessary that the monad be able to accommodate both
kinds of steps. 
...
A \textbf{predomain relation} $c : A \rel A'$ is a relation between $A$ and $A'$
...
property. The authors take as a starting point a semantics with a notion of
representable relations, which in prior work was sufficient for interpreting the
four cast rules crucial for graduality. However, as the authors show, the notion
of representability cannot be applied unchanged to the guarded setting because
of a fundamental tension between transitivity and step-insensitive reasoning. As
a result, Giovannini et al. worked with a weakened notion that they called
\emph{quasi-representability}, which is essentially representability up to an
explicit delay term. They then developed a theory of these delay terms, which
they called \emph{syntactic perturbations}.
...
(generalized) error domains. This means that the constructions employed by
Giovannini et al. will work in our more general setting without any major
modifications.


\subsection{Composition of Casts}
...
function $\run : \term{A} \to T_{db} (\val{A})$ taking a closed term to its
final behavior. Then in Section \ref{sec:denotational-semantics} we defined the
denotation of a term as a function $\sem{\cdot} : \term{A} \to T_d(\sem{A})$.
Our goal is to relate the operational and denotational semantics. Then our goal
is to show that for closed terms of type $\nat$, we have

\[ \run(M) \equiv \sem{M}, \]

where $\equiv$ is a relation between $T_{db}(X)$ and $T_d(X)$ that ignores the
administrative delays in its first argument.

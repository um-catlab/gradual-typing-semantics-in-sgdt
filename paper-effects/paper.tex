...
Despite being a conceptually simple idea, graduality has been a
challenge to prove. The main approaches in prior work are (1) tedious
operational simulation results \cite{siek-stuff} (2) fitting the
language into a particular framework such as abstracting gradual
typing \cite{agt-stuff} or (3) operational logical-relations based
methods \cite{new-ahmed2018} or (4) denotational methods
\cite{new-licata18,gtt-sgdt}. Among these, the advantage of the
denotational approach is a decoupling of the mathematical arguments to
particular syntactic details, so that the lemmas that are core to the
graduality proof can be reused modularly across different
developments. A downside of the denotational approach in prior work
has been that it only proves a graduality statement about a
denotational semantics, and so does not prove the same theorem exactly
as prior graduality results, which are defined over operational
semantics.
...
idea commonly known as \emph{algebraic effects}\cite{alg-effects}. We
...
The foundation for our work is \emph{guarded type theory} which allows
...
interaction trees \cite{trace,itree}. second, we give a
...
The framework is parameterized an \emph{algebraic theory} which
specifies what effectful operations are available (reading from a
...
corresponding to explicit operational reductions steps, marked as
either ``bureaucratic'' reductions or non-well-founded. The result is
...
give two methods for constructing graduality schema.

First, for a \emph{free} theory, meaning a theory with operations but
no equations, then the semantics can be understood as an

...
\begin{example}[Nondeterminism]\label{ex:nondeterminism}
  The theory of nondeterminism $\calA_{\nondet}$ models computations that
  can make nondeterministic choices.
  The signature $\Sigma_{\nondet}$ contains a single operation $\choose$ with $\tau_{\choose} =
  2$ and $\gamma_{\choose} = 0$. The equations specify that the operation is associative, commutative, and idempotent.
...
\begin{example}[Computational Steps]\label{ex:steps}
    ...
    Let $\Sigma_{\step}$ contain a single operation $\step$ with $\tau_{\step} =

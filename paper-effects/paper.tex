...
operational semantics to an effectful gradually-typed cast calculus and proving
that the resulting semantics satisfies graduality. The input to the framework
includes a specification of the algebraic effects present in the cast calculus,
given in the form of an \emph{algebraic theory}, which contains the operations
along with equations that they satisfy. More details on algebraic theories are
given in Section \ref{sec:modeling-effects}. The cast calculus contains a term
corresponding to each of the operations present in the provided algebraic theory
$\calA$, allowing the programmer to perform the effects contained in the theory.
...
effect we call ``bureaucratic delays''. The additional delay effect accounts for
the fact that a priori, we do not know whether the given term will terminate.
...
the monad in which $\run{M}$ lives supports a special ``bureaucratic'' stepping
operation that the monad where $\sem{M}$ lives does not. Bureaucratic steps
correspond to operational rules such as beta reductions that are invisible in
...
The relation $x \equiv_b y$ means that $x$ and $y$ are the same up-to a
\emph{finite} number of bureaucratic steps in $x$. The adequacy theorem then
says that the denotational semantics is equivalent to the operational semantics
modulo finitely many bureaucratic steps.
...
To prove the adequacy theorem, we made an improvement to the denotational
semantics of Giovannini et al. In their semantics, there were additional steps
inserted in the semantics of certain casts. These steps are needed in the
graduality proof, but their presence means that the operational and denotational
semantics are not synchronized. To resolve this, we define a second semantics
for casts that does not include the additional steps. The two semantics are
weakly bisimilar, which is sufficient for graduality. Moreover, the version
without the additional steps can be related in lock-step to the operational
semantics without requiring any modifications to the operational rules.

Having made the aforementioned adjustment to the denotational semantics, we then
prove the adequacy theorem using a logical relation constructed in the ambient
guarded type theory. More details are given in Section
\ref{sec:relating-the-semantics}.
%\max{We should probably say something about the fact that we improve the denotational semantics from prior work in order to make this true}
...
type theory, and the result we obtain is a statement that can be made in
ordinary set theory.
%
